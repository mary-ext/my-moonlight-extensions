...
/**
 * registers the `get_logs` tool.
 *
 * @param registry registry to add the tool to
 */
export const registerGetLogs = (registry: ToolRegistry): void => {
	registry.define({
		name: 'get_logs',
		...
		input: v.object({
			sinceId: v.pipe(
				...
				v.description(
					`Return messages after this ID; use the previous result's nextId to read newer messages`,
				),
			),
		...
		async handler(query) {
			...
			const lines = entries.map(
				(e) => `#${e.id} [${e.level}] ${e.source ? `(${e.source}) ` : ''}${e.message}`,
			);
